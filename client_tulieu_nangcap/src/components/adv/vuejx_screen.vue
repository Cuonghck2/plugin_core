<script setup>
  ...
  const changeArrayRemoveKey = ref({})
  ...
  const submitDataConditon = async (isOk, action) => {
    console.log('submitDataConditonsubmitDataConditonsubmitDataConditon', isOk, action);
    if (isOk) {
      await submitData(action)
    }
  }
  const submitData = async (action, doneExt) => {
    afterSubmit.value = true;
    console.log('action', action, action.emit, dataForm.value);
    if (action.emit) {
    ...
    if (Array.isArray(formJSON.value)) {
      for (let el of formJSON.value) {
        if (el.required && !dataForm.value[el.model] && el.type != 'label' && String(el.class).indexOf('hidden') === -1) {
          console.log('el valid =====', el);
          valid = false;
          break
        }
      }
    ...
    setTimeout(async () => {
      if (!valid && !action.novalid) {
        window.Vue.toastr.error({
          message: "Kiểm tra các trường thông tin bắt buộc."
        });
  ...
  const initDataDate = (data) => {
    dataForm.value[data.key] = data.data
  }  
  ...
  const processArrayDelete = async (index, model) => {
    if (confirm('Bạn muốn xoá bản ghi này?')) {
      changeArrayRemoveKey.value[model] = new Date().getTime()
      dataForm.value[model].splice(index, 1);
      console.log('changeArrayRemoveKey.value[model]', changeArrayRemoveKey.value[model]);
    }
  }
  ...
  const changeRole = async () => {
    ...
    console.log('changeRole dataForm.valuedataForm.value', dataForm.value);
  }
  ...
  const setDemoAction = (action) => {
    props.demoaction = action
  }
  defineExpose({
    changeData,
    dataForm, formJSON,
    submitData, submitDataAction,
    submitDataConditon, setDemoAction
  })
...
<template>
  <div class="dynamic__form__vuejx___wrap relative z-50" :class="{'done___loading': renderPageLoadView}">
        ...
        <template v-if="renderScreen">
          ...
          <div :class="item['class']" v-for="(item, index) in formJSON" v-bind:key="index">
            ...
            <div v-else-if="item['type'] === 'file'" :class="'vuejx_comp___' + item['model']">
                ...
                <vuejx-upload :id="item['model']" :class="item['upload_class']" :editable="item['editable']"
            ...
            <div v-else-if="item['type'] === 'number'" :class="'vuejx_comp___' + item['model']">
              ...
              <div v-if="afterSubmit && item['required'] && !dataForm[item.model] && String(dataForm[item.model]) == ''" v-show="afterSubmit" class="text-red-600 mt-2 italic error___form"> {{ item.required_title ? item.required_title : 'Trường dữ liệu bắt buộc' }}</div>
            </div>
            <vuejx-boolean v-else-if="item['type'] === 'boolean'" :item="item" :dataForm="dataForm" @changeData="dataChange(item['model'])"></vuejx-boolean>
            ...
            <div v-else-if="item['type'] === 'table'" :class="'vuejx_comp___' + item['model']">
                ...
                >
                      ...
                      <template v-slot:tbody="{ data }">
                        <template v-if="dataForm[item.model] && !searchablePick[item.model] && !item.formConfigEdit && !item.autoShow">
                        ...
                        <template v-else-if="searchablePick[item.model] || item.autoShow">
                ...
                <slot :name="'array_' + item['model']" v-bind:dataArrayModel="dataForm[item.model]" v-bind:processArrayDelete="processArrayDelete" v-bind:dataFormX="dataForm" v-bind:item="item">
                  ...
                  <div class="relative" v-for="(itemDataArray, indexAr) in dataForm[item.model]" v-bind:key="indexAr">
                    ...
                    <vuejx-screen class="z-10" :ref="'formInputArray_' + indexAr" :grid_class="item.grid_class" :config="item" 
                      ...
                      :data="flattenKeysData(itemDataArray)" :key="changeArrayRemoveKey[item.model]">
                        <template v-slot:action="{ modelx, dataFormX }" >
                          <span></span>
                        </template>
                    </vuejx-screen>
            ...
            <div v-else-if="item['type'] === 'role'">
              ...
              }" :data="dataForm" v-model="dataForm['viewRole']"
                @change="changeRole"
              ></vuejx-autocomplete>
              <vuejx-autocomplete :config="{
                modelView: 'object', itemText: '_source.TenMuc', itemValue: '_source.MaMuc',
                ...
                link: [{ collection: 'C_ROLE' }],
                column: ['MaMuc', 'TenMuc', 'type'],
              }" :data="dataForm" v-model="dataForm['ignoreRole']"
                @change="changeRole"
              ></vuejx-autocomplete>
            </div>
    ...
    <slot name="action" v-bind:submitData="submitData" v-bind:dataFormX="dataForm">
      <div class="btn-group flex mt-8 action___buttons" v-if="renderScreen && !action_guest" :key="demoaction">
      ...
      <div class="btn-group flex mt-8 action___buttons" :key="demoaction" v-else>
        <template v-for="(item, index) of demoaction">
          ...
          <div :class="detailActin.ext_field.class" v-if="Array.isArray(detailActin.ext_field.value)">
            <div :class="item['class']" v-for="(item, index) in detailActin.ext_field.value" v-bind:key="index">
              ...
              <div v-else-if="item['type'] === 'number'" :class="'vuejx_comp___' + item['model']">
                ...
                <div v-if="afterSubmit && item['required'] && !dataForm[item.model] && String(dataForm[item.model]) != '0'" v-show="afterSubmit" class="text-red-600 mt-2 italic error___form"> {{ item.required_title ? item.required_title : 'Trường dữ liệu bắt buộc' }}</div>
              </div>
              <vuejx-boolean v-else-if="item['type'] === 'boolean'" :item="item" :dataForm="dataForm" @changeData="dataChange(item['model'])"></vuejx-boolean>
              <vuejx-region v-else-if="item['type'] === 'region'"
                :item="item" :dataForm="dataForm" @changeData="changeData"
              ...
              <div class="-mt-4" v-else-if="item['type'] === 'array' && Array.isArray(item.form)">
                <div class="block___label">
                  ...
                  <div class="relative" v-for="(itemDataArray, indexAr) in dataForm[item.model]" v-bind:key="indexAr">
                    ...
                    <vuejx-screen class="z-10" :ref="'formInputArray_' + indexAr" :grid_class="item.grid_class" :config="item" 
                      :modelForm="item.model" :modelFormIndex="indexAr" @dataChangeArrayForm="changeDataFormArray" 
                      :data="itemDataArray" :key="changeArrayRemoveKey[item.model]">
                        <template v-slot:action="{ modelx, dataFormX }" >
                          <span></span>
                        </template>
                    </vuejx-screen>
