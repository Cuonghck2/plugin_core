export default {
    ...
    methods: {
        ...
        doLogout () {
            try {
                //localStorage.clear();
                window.saveToIndexedDB('screens', { page: [], id: 'groupNav' });
                window.saveToIndexedDB('screens', { page: [], id: 'webNav' });
        ...
        async pullNewPages(nav) {
            let vm = this;
            if (nav) {
                await window.db.get(nav).get().then(
                async(document) => {
                    if (document) {
    ...
    template: `
    <div v-if="render___header__nav">
        <slot name="menu_top_right">
            <Teleport to="#body___container__header_view">
                <span></span>
            </Teleport>
        </slot>
    </div>
    <nav class="tree-nav" :key="pages">
        <details
            v-for="(item, index) in pages" v-bind:key="index" tabindex="0"
            :open="page ? page.startsWith(Object.values(item)[0].shortName) || String(Object.values(item)[0].activePage).indexOf(page + ',') !== -1 : false"
            class="tree-nav__item is-expandable"
            :class="{ active: page ? page.startsWith(Object.values(item)[0].shortName) || String(Object.values(item)[0].activePage).indexOf(page + ',') !== -1 : false }"
        >
            <summary
                @click="toPage(Object.values(item)[0])"
                class="tree-nav__item-title relative"
                :class="{ active: page ? page.startsWith(Object.values(item)[0].shortName) || String(Object.values(item)[0].activePage).indexOf(page + ',') !== -1 : false }"
            >
                <i class="mdi icon_nav" :class="Object.values(item)[0].shortName"></i>
                <span>{{ Object.values(item)[0].title }}</span>
                <svg v-if="Object.values(item)[0].sub___pages && Object.values(item)[0].sub___pages.length > 0" class="absolute right-0 expand___tree" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M19.9201 8.94995L13.4001 15.47C12.6301 16.24 11.3701 16.24 10.6001 15.47L4.08008 8.94995" stroke="#8A8E95" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
            </summary>
            <div class="wrap__tree__child">
                <details  v-if="Object.values(item)[0].sub___pages && Object.values(item)[0].sub___pages.length > 0"
                    v-for="(itemChild, indexChild) in Object.values(item)[0].sub___pages"
                    v-bind:key="indexChild"
                    class="tree-nav__item is-expandable"
                >
                    <summary
                        @click="toPage(Object.values(itemChild)[0])"
                        class="tree-nav__item-title flex"
                        :class="{ active: page ? page.startsWith(Object.values(itemChild)[0].shortName) || String(Object.values(itemChild)[0].activePage).indexOf(page + ',') !== -1 : false }"
                    >
                        <i class="mdi mdi-chevron-right"></i>
                        <span>{{ Object.values(itemChild)[0].title }}</span>
                    </summary>
                </details>
            </div>
        </details>
