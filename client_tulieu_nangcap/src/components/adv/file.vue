  ...
  export default {
    ...
    methods: {
      ...
      async doRemoveFile() {
          ...
          if (r == true) {
              for (let el of vm.idFileToRemoves) {
                  await window.Vue.$axios.post('/security/file/upload_remove/' + el['bucketName'] + '/' + el['filename'], {}, {
                      headers: {
      ...
      previewFile(bucketName, filename) {
          window.open("/security/file/" + bucketName + "/" + filename);
      }
    },
    mounted: function() {
        let vm = this;
