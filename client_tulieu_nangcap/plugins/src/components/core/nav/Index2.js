export default {
        ...
        async pullNewPages(nav) {
            ...
            if (nav) {
                ...
                if (document) {
                    ...
                    let siteNav = await window.initPage(window.Vue.router.currentRoute.value.params.visibility)
                    vm.pages = siteNav[nav];
                }
                let isAdmin = false;
        ...
        '$route': async function(newRoute, oldRoute) {
            ...
            if (newRoute.params.visibility === 'group' || newRoute.params.visibility === 'web') {
                if (newRoute.params.visibility != oldRoute.params.visibility) {
                    console.log('watchwatchwatchNAV');
                    vm.renderPage = false;
                    vm.pages = [];
    ...
    template: `<div class="vuejx__header"><slot name="header_top"></slot><div v-if="defaultNav" :id="teleport" class="hd__screen"></div><template v-if="renderPage"><Teleport :to="'#' + teleport"><div><div class="top-bar"><slot name="header_nav"><div class="left" tabindex="0"><template v-for="(item, index) in pages"><div v-bind:key="index" class="item" tabindex="0" :class="{ active: page ? page.startsWith(Object.values(item)[0].shortName) || String(Object.values(item)[0].activePage).indexOf(page + ',') !== -1 : false, 'mr-2': Object.values(item)[0].sub___pages && Object.values(item)[0].sub___pages.length > 0 }"v-if="item && Object.values(item) && Object.values(item)[0] && Object.values(item)[0].perView"><button aria-label="btn" class="label" @click.stop="toPage(Object.values(item)[0])"> {{ Object.values(item)[0].title }} <i v-if="Object.values(item)[0].sub___pages && Object.values(item)[0].sub___pages.length > 0" class="mdi mdi-chevron-double-down" style="margin-top: -2px;"></i> </button><div class="dropdown" v-if="Object.values(item)[0].sub___pages && Object.values(item)[0].sub___pages.length > 0"> <div v-for="(sub, indexSub) in Object.values(item)[0].sub___pages" v-bind:key="indexSub"> <button aria-label="btn" class="dropdown-child" @click.stop="toPage(sub['_source'])"> {{ sub._source?.title }} </button> </div> </div></div></template></div></slot><slot name="header_login"><div class="left" tabindex="0"><div class="item"><button aria-label="btn" class="label" v-if="user"> <i class="mdi mdi-account-circle" /> <span>{{ user?.title }}</span> </button><button aria-label="btn" class="label" @click="toLogin" v-else> <i class="mdi mdi-power-settings-new" /> <span>Đăng nhập</span> </button><div class="dropdown" v-if="user"><div @click="toGroupPage('web')"> <button aria-label="btn" class="dropdown-child">Trang khai thác</button> </div><div> <div class="divider"></div> </div><div @click="toGroupPage('group')"> <button aria-label="btn" class="dropdown-child">Trang quản trị</button> </div><div> <div class="divider"></div> </div><div @click="changePass = !changePass"> <button aria-label="btn" class="dropdown-child">Đổi mật khẩu</button> </div><div> <div class="divider"></div> </div><div @click="doLogout"> <button aria-label="btn" class="dropdown-child">Đăng xuất</button> </div></div></div></div></slot></div></div><div class="fixed z-10 inset-0 overflow-y-auto" style="z-index: 9999;" v-if="changePass"><div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0"><div class="fixed inset-0 transition-opacity"><div class="absolute inset-0 bg-gray-100 opacity-75"></div></div><span class="hidden sm:inline-block sm:align-middle sm:h-screen"></span><div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle w-1/4"role="dialog"aria-modal="true"aria-labelledby="modal-headline"style="z-index: 9999;"><div class="bg-white px-4 py-2 border-b"><div class="sm:flex sm:items-start"><div class="text-center sm:mt-0 sm:text-left"><h3class="text-lg leading-6 font-semibold text-gray-900"id="modal-headline">Đổi mât khẩu</h3></div></div></div><div class="p-4"><div><div class="block font-semibold leading-normal truncate">Mật khẩu mới<spanclass="required__class">*</span></div><inputv-model="passwordNew"class="p-2 focus:cursor-text w-full border border-gray-200 bg-gray-200 text-gray-700 focus:outline-none focus:bg-white focus:border-gray-400 rounded"type="password"placeholder="Mật khẩu mới"/></div><div><div class="block font-semibold leading-normal truncate">Xác nhận mật khẩu<spanclass="required__class">*</span></div><inputv-model="passwordConfirm"class="p-2 focus:cursor-text w-full border border-gray-200 bg-gray-200 text-gray-700 focus:outline-none focus:bg-white focus:border-gray-400 rounded"type="password"placeholder="Mật khẩu mới"/></div></div><div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse"><span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto"><button @click="doChangePassAction"type="button"class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-red-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red transition ease-in-out duration-150 sm:text-sm sm:leading-5">Xác nhận</button></span><spanclass="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto"><button @click="changePass = !changePass"type="button"class="inline-flex justify-center w-full rounded-md border border-gray-300 px-4 py-2 bg-white text-base leading-6 font-medium text-gray-700 shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5">Quay lại</button></span></div></div></div></div></Teleport></template></div>`
}