<script setup>
  ...
  const flattenKeysData = (data) => {
    let diaChiObj = {}
    diaChiObj[props.item.model] = data
  ...
  const processArray = async (data, model, config) => {
    ...
    let valid = true
    if (valid) {
      if (Array.isArray(props.dataForm[model])) {
        props.dataForm[model].push(dataClean[props.item.model])
      } else {
        props.dataForm[model] = [dataClean[props.item.model]];
      }
    } else {
  ...
  const changeDataInput = async (data) => {
    console.log('changeDataInput', data.data, data.model, props.item.model + '.' + data.model);
    emit('changeData', {
      type: 'autocomplete',
      model: data.model,
      data: data.data
      ...
      <div :key="renderArrayForm">
        <div class="relative" v-for="(itemDataArray, indexAr) in dataForm[item.model]" v-bind:key="indexAr">
          ...
          <vuejx-screen class="z-10" ref="formInputArray" :grid_class="item.grid_class ? item.grid_class : 'grid grid-cols-3 gap-2'" :config="{
            ...
            :modelForm="item.model" :modelFormIndex="indexAr" @dataChange="changeDataFormArray" 
