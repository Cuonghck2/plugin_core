    ...
    methods: {
        objectView(item, key, defaultVal) {
          ...
          async init() {
            ...
            console.log('vm.valuevm.valuevm.valuevm.valuevm.valuevm.value', vm.value, vm.options, vm.dynamicAPI, window.Vue.router.currentRoute.value.query[vm.config['model']]);
            if (vm.dynamicAPI && window.Vue.router.currentRoute.value.query[vm.config['model']] && vm.options) {
                vm.value  = vm.options[0]
                console.log('vm.valuevm.valuevm.valuevm.valuevm.valuevm.value', vm.value, vm.options);
            }
          },
          ...
          async pullDataSource(keyword) {
              let vm = this;
              console.log('vm.configvm.config', vm.config, keyword);
